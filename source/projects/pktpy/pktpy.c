...
t_class* pktpy_class = NULL; // global pointer to object class
...
{
    t_class* c = class_new("pktpy", (method)pktpy_new,
                           (method)pktpy_free, (long)sizeof(t_pktpy), 0L,
                           A_GIMME, 0);

    class_addmethod(c, (method)pktpy_bang,  "bang", 0);
    class_addmethod(c, (method)pktpy_float, "float", A_FLOAT, 0);
    ...
    CLASS_ATTR_ACCESSORS(c, "name", pktpy_name_get, pktpy_name_set);
...
{
    t_pktpy* x = (t_pktpy*)object_alloc(pktpy_class);
...
t_max_err pktpy_handle_output(t_pktpy* x, py_GlobalRef retval)
{
    ...
    if (py_isfloat(retval)) {
        return pktpy_handle_float_output(x, retval);
    }

    else if (py_isint(retval)) {
        return pktpy_handle_long_output(x, retval);
    }

    else if (py_isstr(retval)) {
        return pktpy_handle_string_output(x, retval);
    }

    else if(py_isbool(retval)) {
        return pktpy_handle_bool_output(x, retval);
    }

    else if(py_islist(retval)) {
        return pktpy_handle_list_output(x, retval);
    }

    else if(py_istuple(retval)) {
        return pktpy_handle_tuple_output(x, retval);
    }

    // else if (py_isdict(retval)) {
    //     return pktpy_handle_dict_output(x, retval);
    // }
...
t_max_err pktpy_handle_float_output(t_pktpy* x, py_GlobalRef pfloat)
{
    ...
    if (py_checkfloat(pfloat)) {
        ...
        pktpy_bang_success(x);
    }
    return MAX_ERR_NONE;

error:
    object_error((t_object*)x, "pktpy_handle_float_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
...
error:
    object_error((t_object*)x, "pktpy_handle_long_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
...
error:
    object_error((t_object*)x, "pktpy_handle_string_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
...
error:
    object_error((t_object*)x, "pktpy_handle_bool_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
...
t_max_err pktpy_handle_list_output(t_pktpy* x, py_GlobalRef plist)
{
    ...
    if (py_checklist(plist)) {
        ...
        while (true) {
            ...
            if (res == ITER_FAILURE) {
                object_error((t_object*)x, "pktpy_handle_list_output iter failed");
                goto error;
            }
...
error:
    object_error((t_object*)x, "pktpy_handle_list_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
...
t_max_err pktpy_handle_tuple_output(t_pktpy* x, py_GlobalRef ptuple)
{
    ...
    if (py_checktuple(ptuple)) {
        ...
        while (true) {
            int res = py_next(py_getreg(0));
            if (res == ITER_FAILURE) {
                object_error((t_object*)x, "pktpy_handle_tuple_output iter failed");
                goto error;
            }
...
error:
    object_error((t_object*)x, "pktpy_handle_tuple_output failed");
    pktpy_bang_failure(x);
    return MAX_ERR_GENERIC;
}
 ...
 * @return t_max_err error code
 */
// t_max_err pktpy_handle_dict_output(t_py* x, py_GlobalRef pdict)
// {
//     PyObject* pfun = NULL;
//     PyObject* pval = NULL;
...
// error:
//     object_error((t_object*)x, "pktpy_handle_dict_output failed");
//     py_bang_failure(x);
//     return MAX_ERR_GENERIC;
// }
...
/**
 * @brief Execute a max symbol as one to many lines of pktpy code
 *
 * @param x pointer to object structure
 * @param s symbol
...
void pktpy_doread(t_pktpy* x, t_symbol* s, long argc, t_atom* argv)
{
    ...
    pktpy_locate_path_from_symbol(x, s);
...
t_max_err pktpy_execfile(t_pktpy* x, t_symbol* s)
{
    ...
    if (s != gensym("")) {
        ...
        err = pktpy_locate_path_from_symbol(x, s);
        if (err != MAX_ERR_NONE) {
...
void pktpy_float(t_pktpy *x, double f)
{
    ...
    default:
        error("pktpy_float switch out-of-index");
    }
}
 ...
 * @param x pointer to object struct.
 */
void pktpy_bang_success(t_pktpy* x) { outlet_bang(x->p_outlet_right); }
 ...
 * @param x pointer to object struct.
 */
void pktpy_bang_failure(t_pktpy* x) { outlet_bang(x->p_outlet_middle); }
    ...
    if (argc && argv) {
        if (atom_alloc(argc, argv, &alloc)) {
            ...
            if (alloc) {
                ...
                post("pktpy_name_get: %s", x->name->s_name);
        }
    }
    return MAX_ERR_NONE;
}
...
t_max_err pktpy_name_set(t_pktpy *x, t_object *attr, long argc, t_atom *argv)
{
    if (argc && argv) {
        ...
        post("pktpy_name_set: %s", x->name->s_name);
    }
    return MAX_ERR_NONE;
}
